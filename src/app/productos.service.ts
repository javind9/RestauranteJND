...
import { ProductoModel } from './components/models/producto.model';
...
export class ProductosService {
    ...
    private urlCarrito = 'https://restaurante-fd301-default-rtdb.firebaseio.com';
    ...
    this.getPlatos().subscribe( resp =>{
        this.productos = resp;
      });

      this.getBebidas().subscribe( resp =>{
        this.productos = resp;
      });

      this.getPostres().subscribe( resp =>{
        this.productos = resp;
      });
    }
    ...
    getPlatos(){
        //return this.platos;
        return this.http.get(`${this.url}/platos.json`).pipe(
            map(this.crearArreglo)
        );
    }

    getBebidas(){
        return this.http.get(`${this.url}/bebidas.json`).pipe(
            map(this.crearArreglo)
        );
    }
    ...
    getCarrito(){
        return this.http.get(`${this.urlCarrito}/carrito.json`).pipe(
            map(this.crearArreglo)
        );
    }

    borrarProducto(id: string){
        return this.http.delete(`${this.urlCarrito}/carrito/${id}.json`);
    }

    crearProducto(producto: ProductoModel){
        return this.http.post(`${this.urlCarrito}/carrito.json`, producto);
    }


    //Extraer un objeto para mostrarlo
